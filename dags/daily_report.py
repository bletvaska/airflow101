...
def extract_yesterday_data(logical_date: pendulum.DateTime) -> DataFrame:
    ...
    try:
        ...
        filtered_data = df.loc[
            tf_yesterday & tf_today, ["dt", "city", "temp", "humidity"]
        ]
        # print(filtered_data)
...
def create_report(data: DataFrame):
    ...
    bucket.upload_file(path, f"{city.lower()}.txt")
...
@task
def notify(logical_date: pendulum.DateTime):
    yesterday = logical_date.start_of("day").subtract(days=1).naive()
    ...
    apprise.notify(
        title="Upozornenie!",
        body=f"Nový report za deň {yesterday.to_date_string()} je pripravený.",
    )
...
def main(hello: str = "world"):
    ...
    create_report(extracted_data) >> notify()
